...
export class ReporteHistorialAcademicoExcelService {
  logo: any;
  ...
  exportExcel(excelData: { title: any; headers: any; data: any }) {
    ...
    let worksheet = workbook.addWorksheet('HistorialAcademico');
    worksheet.columns = [
      { width: 10 },
      { width: 40 },
      { width: 70 },
    ...
    titulo.value = 'Reporte: Formación Académica';
    titulo.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: '8F141B' },
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      saveAs(
        blob,
        title + 'Foración Académica ' + fecha + 'xlsx'
      );
    });
  }
}

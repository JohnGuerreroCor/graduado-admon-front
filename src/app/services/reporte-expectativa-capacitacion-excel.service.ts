...
export class ReporteExpectativaCapacitacionExcelService {
  logo: any;
  fecha = this.datePipe.transform(Date.now(), 'dd-MM-yyyy h:mm a');
  ...
  exportExcel(excelData: { title: any; headers: any; data: any }) {
    ...
    let worksheet = workbook.addWorksheet('ExpectativaCapacitacion');
    worksheet.columns = [
      { width: 10 },
      { width: 40 },
      { width: 70 },
    ...
    titulo.value = 'Reporte: Expectativas de Capacitación y Formación';
    titulo.fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: '8F141B' },
    ...
    workbook.xlsx.writeBuffer().then((data) => {
      ...
      saveAs(
        blob,
        title + 'Expectativas de Capacitación y Formación ' + fecha + 'xlsx'
      );
    });
  }
}

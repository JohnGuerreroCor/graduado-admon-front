...
export class SoporteExpedicionService {
  ...
  registrarSoporte(archivo: File, json: SoporteExpedicion): Observable<null> {
    ...
    return this.http.post<null>(`${this.url}/registrar-soporte-expedicion/${this.userLogeado}/${this.perCodigo}/${this.uaa}`, formData, { headers: this.aggAutorizacionHeader() });
  }

  actualizarSoporte(soporte: SoporteExpedicion): Observable<number> {
    return this.http.put<number>(`${this.url}/actualizar-soporte-expedicion/${this.userLogeado}`, soporte, { headers: this.aggAutorizacionHeader() });
  }
  ...
  enviarEmail(email: String): Observable<any> {
    return this.http.get<any>(`${this.url}/enviar-email-rector/${email}`, { headers: this.aggAutorizacionHeader() });
  }
}
